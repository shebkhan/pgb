...
MerchantCarousel = function(data){
	...
	$.each(data, function(key, val){
	    ...
	    html +='<ons-ripple color="#ef662500" background="#ef662500"></ons-ripple>';
		    html +='<div class="banner">';
			    ...
			    if (val.minimum_order_raw>0){
				    /*html +='<div class="min_tag">';
				     html += val.minimum_order+'<br><small></small>';
				    html +='</div>';*/
			    }
...
restoTabMenu = function(data){
	...
	$.each( data  , function( key, val ) {
		...
		html+='<ons-carousel-item '+is_selected+' onclick="setClickTab('+ "'"+  val.page_name +"'," +  x  +')" >';
	     html+=val.label;
	    html+='<ons-ripple color="#ef662500" background="#ffffff"></ons-ripple>';
	    html+='</ons-carousel-item>';
	    x++;
	});
	html+='</ons-carousel>';
...
var displayItemDetails = function(data, cart_data) {
	...
	html+='<div class="item_previeww" style="background-image: url('+ "'" + addslashes(data.photo) + "'" +')"  >';
...
var displayCartDetails = function(datas){
	...
	html+='<ons-list-item tappable modifier="chevron longdivider" onclick="showDeliveryDateList()" style="display:none;" >';	
...
setItemListColumn = function(data, element){
	...
	$.each( data  , function( key, val ) {
         ...
         col+='<ons width="100%" vertical-align="top" onclick="itemDetails('+ "'"+ val.item_id+"'," + "'" + val.cat_id + "'"  +')"  >';
			col+='<div class="bannerr">';
			  ...
			  col+='</div>';
			col+='</div> ';
			col+='<div class="item_text">';
			col+='<h5>'+ val.item_name +'</h5>';
			col+='</div> ';
			...
			col+='<p class="concat_text" style="width:355px;" >'+ val.item_description +'</p>';
...
fillOrderTabs = function(element, tab, selected_index){	
	...
	if(app_settings = getAppSettings()){
		...
		$.each( app_settings.order_tabs  , function( key, val ) {
			...
			html+='<ons-carousel-item class="'+ is_selected +'" onclick="OrderListTab('+ "'" + key + "'," + index  +')"  >'+ t(val) +'<ons-ripple color="#ef662500" background="#ffffff"></ons-ripple></ons-carousel-item>';						
	        index++;
...
fillBookingTabs = function(element, tab, selected_index){
	...
	if(app_settings = getAppSettings()){
		...
		$.each( app_settings.booking_tabs  , function( key, val ) {
			...
			html+='<ons-carousel-item class="'+ is_selected +'" onclick="BookingListTab('+ "'" + key + "'," + index  +')"  >'+ t(val) +'<ons-ripple color="#ef662500" background="#ffffff"></ons-ripple></ons-carousel-item>';						
	        index++;
...
addTermsCondition = function(element){
	 ...
	 html+='<span class="trn">'+ t("I Agree To The") +'</span>&nbsp;<a href="javascript:;" class="small" onclick="loadCustomPage(2)">';
